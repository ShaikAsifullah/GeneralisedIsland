...
def differential_evolution(func, bounds, args=(), strategy='best1bin',
                           ...
                           island_marker=4,number_of_migrations=4,number_of_islands=4,
                           key="MILAND",
                           wait_time=10, poll_time=3
                           ):
    ...
    solver = DifferentialEvolutionSolver(func, bounds, args=args,
                                         ...
                                         island_marker=island_marker,number_of_migrations=number_of_migrations,number_of_islands=4,
                                         key=key,
                                         wait_time=wait_time, poll_time=poll_time
                                         )
    return solver.solve()
...
class DifferentialEvolutionSolver(object):
    ...
    def solve(self):
        ...
        for nmig in xrange(1,self.number_of_migrations+1):
            ...
            # Check if something is already cached if it is not the first migration
            if nmig != 1:
                neighbour = (int(self.island_marker[-1]) + 1) % self.number_of_islands
                replacement = client.get(self.key + str(neighbour))
                ...
                if replacement is not None:
                    print(replacement)
                    self.population[1] = np.array([float(items) for items in replacement.split(",")])
                    self.population_energies[1] = self.func(self.population[1],
                                                        *self.args)
...
def beale_function(x):
    t1 = (1.500 - x[0] + x[0] * x[1]) ** 2
    t2 = (2.250 - x[0] + x[0] * x[1] ** 2) ** 2
